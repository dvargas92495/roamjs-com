...
contributors: Bardia Pourvakil 💵📓, Isabela Granic 📓🤔
---
...
You could manually trigger the daily random block reference query by adding a `{{serendipity}}` block on a given daily note page and clicking the button. There is also a `Run Serendipity` command available from the command palette.
